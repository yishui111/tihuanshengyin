...
| **训练音色**（一键换声） | 8011 | 原视频/音频 → 只换音色为训练好的真人音色，**音高/语调/时长保持原样**；模型放 `rvc_service\models\<角色>\`（训练中心交付包整个文件夹复制进来即自动识别） |
| **功能A** RVC 二次元角色换声 | 8011 | 素材 + 角色名 → 换成该角色音色（保留语速/停顿/情绪），角色 .pth 放 `rvc\assets\weights\` |
...
- 🔊 **自动防破音**：原素材声音过大时自动压电平（进引擎前、单段后、成品三级），成品峰顶留编码余量，大音量视频换声不再爆音；
...
| ECAPA 声纹模型（speechbrain `spkrec-ecapa-voxceleb`，回退后端） | 工作台说话人检测兜底 | 联网机器运行 `runtime\py312\python.exe hub\download_ecapa.py` 一次性下载到 `runtime\cache\`（离线放置法见 DEPLOY.md） |
